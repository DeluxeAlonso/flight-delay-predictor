...
class Main:
    ...
    def random_forest_grid_search(self):
        training_filename = 'Datasets/RandomForestDataset.csv'
        training_dataset = Utils.load_processed_dataset(training_filename)
    ...
    def random_forest_predictor(self):
        ...
        testing_dataset = Utils.load_processed_dataset(testing_filename)
        random_forest_property_length = len(training_dataset[0].get_random_forest_property_array())
        predictions, probabilities = RandomForest.get_sklearn_predictions(training_dataset, testing_dataset,
                                                   random_forest_property_length)
        eval.show_evaluation_measure_values(testing_dataset, predictions)
        eval.show_roc_curve(testing_dataset, probabilities)

    def data_exploration(self):
        filename = 'Datasets/NaiveBayesDataset.csv'
        dataset = Utils.load_processed_dataset(filename)
